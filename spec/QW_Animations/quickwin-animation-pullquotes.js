...
const quickWinGroups = [
  ...
  { parts: [
    ...
    { type: 'pull', text: 'AI knows the celebrity story. It has no idea about the rest.' },
    ...
    { type: 'pull', text: 'AI recommends whoever provides context — not whoever has better work.' },
    ...
    { type: 'pull', text: 'The work is exceptional. The site is only telling part of the story.' },
...
function renderPartInstant(bubble, part) {
  if (part.type === 'header') {
    ...
    el.textContent = part.text;
    bubble.appendChild(el);
  } else if (part.type === 'pull') {
    const el = document.createElement('div');
    el.className = 'qw-pull';
    el.textContent = part.text;
    bubble.appendChild(el);
