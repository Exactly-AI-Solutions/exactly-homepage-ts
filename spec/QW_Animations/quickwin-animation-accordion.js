...
const quickWinGroups = [
  ...
  { parts: [
    { type: 'richbody', html: 'The results are a split picture — and <strong>the split itself is the problem.</strong> For the celebrity query, Reunited shows up. For the private label and turnkey queries, Reunited does not appear. <strong>AI knows the celebrity story. It has no idea about the rest.</strong>' },
    { type: 'accordion', sections: [
      { label: 'The AI Test', parts: [
        ...
        { type: 'body', text: "Every mention traces back to that single Fashionista article. A journalist described Reunited's business more clearly than Reunited describes it itself." },
      ]},
      { label: 'Why AI only knows half of you', parts: [
        { type: 'bullets', lines: [
          'The homepage is invisible to AI: The site is almost entirely images. No client names. No proof points. No category phrases AI can parse.',
          'No private label classification language exists: The site never says "private label apparel manufacturer for major department stores." Without that language, Reunited doesn\'t exist in AI\'s map of the private label industry.',
          'Proof is hidden behind aspirational language: It never says "Launched Royalty by Maluma exclusively at Macy\'s" or "Won Revolve\'s Influencer Collaboration of the Year with Olivia Culpo." The credentials are real. The site doesn\'t say so.',
          'Competitors name-drop; Reunited doesn\'t: Celebrity Fashion Group lists Cristiano Ronaldo, Selena Gomez, and the Kardashians. AI recommends whoever provides context — not whoever has better work.',
          'One article is carrying the entire visibility load: The Fashionista piece is doing the classification work the website should be doing. A single third-party article is not enough to hold the position.',
        ]},
      ]},
      { label: 'What to fix', parts: [
        { type: 'numhead', text: '1.' },
        { type: 'body', text: 'State the full category in text AI can read. "Reunited Clothing is a New York-based design house and apparel manufacturer that builds and launches private label and celebrity brands for major retailers including Macy\'s." That sentence doesn\'t exist on the site.' },
        { type: 'numhead', text: '2.' },
        { type: 'richbody', html: 'Name the work. "Royalty by Maluma — launched exclusively at Macy\'s, 2022. Nina Parker — 2021. Marled by Olivia Culpo — Revolve Award." <strong>Specifics are classification signals. Aspirational language is not.</strong>' },
        { type: 'numhead', text: '3.' },
        { type: 'body', text: 'Make capabilities structurally discoverable. Each capability — design, sourcing, manufacturing, logistics — needs a distinct, indexable section connected to proof.' },
        { type: 'numhead', text: '4.' },
        { type: 'richbody', html: 'Close the competitor gap. <strong>The site needs to say exactly that.</strong>' },
      ]},
      { label: 'The bottom line', parts: [
...
function renderPart(bubble, part, msgDiv, onDone) {
  ...
  } else if (part.type === 'accordion') {
    const wrap = document.createElement('div');
    wrap.className = 'qw-accordion';
    part.sections.forEach(function(sec, idx) {
      const accItem = document.createElement('div');
      accItem.className = 'qw-acc-item';
      const trigger = document.createElement('button');
      trigger.className = 'qw-acc-trigger' + (idx === 0 ? ' acc-open' : '');
      trigger.onclick = function() { qwAccToggle(this); };
      trigger.innerHTML = '<span class="qw-acc-label">' + sec.label + '</span><span class="qw-acc-chevron">&#9662;</span>';
      const body = document.createElement('div');
      body.className = 'qw-acc-body' + (idx === 0 ? ' acc-open' : '');
      (sec.parts || []).forEach(function(p) { renderPartInstant(body, p); });
...
function qwAccToggle(btn) {
  var body = btn.nextElementSibling;
  body.classList.toggle('acc-open');
